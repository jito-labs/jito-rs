...
use crate::SearcherClientResult;
...
impl ClientInterceptor {
    pub async fn new(
        ...
        keypair: &Arc<Keypair>,
    ) -> SearcherClientResult<Self> {
        const ROLE: Role = Role::Searcher;
        ...
        tokio::spawn(Self::token_refresh_loop(
            auth_service_client,
            bearer_token.clone(),
            refresh_token,
    ...
    async fn auth(
        ...
        keypair: &Keypair,
        role: Role,
    ) -> SearcherClientResult<(Token, Token)> {
    ...
    async fn token_refresh_loop(
        ...
        loop {
            ...
            let does_refresh_token_expire_soon = refresh_token_ttl < Duration::from_secs(5 * 60);
