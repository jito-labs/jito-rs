...
use log::*;
use solana_client::nonblocking::tpu_client::{TpuClient, TpuSenderError};
use solana_sdk::{
    clock::Slot, pubkey::Pubkey, transaction::Transaction, transport::TransportError,
};
use thiserror::Error;
use tokio::sync::{
...
use crate::convert::{proto_packet_from_transaction, transaction_from_packet};
...
pub enum SearcherClientError {
    ...
    RpcTransportError(#[from] TransportError),

    #[error("tpu client error {0}")]
    TpuClientError(#[from] TpuSenderError),
}
    ...
    <T::ResponseBody as Body>::Error: Into<StdError> + Send,
{
            ...
            .send_bundle(SendBundleRequest {
                bundle: Some(Bundle {
                    ...
                    packets: transactions
                        .iter()
                        .map(proto_packet_from_transaction)
                        .collect(),
                }),
            })
            .await?;

        Ok(Uuid::from_str(&resp.into_inner().uuid)
            .map_err(|_e| SearcherClientError::InvalidBundleUuid)?)
    }
    ...
    pub async fn send_transactions(
        ...
        let futs = transactions
            .iter()
            .map(|tx| self.tpu_client.try_send_transaction(tx))
            .collect::<Vec<_>>();
    ...
    pub async fn subscribe_mempool_accounts(
        ...
        accounts: &[Pubkey],
        buffer_size: usize,
    ) -> SearcherClientResult<Receiver<Vec<Transaction>>> {
        ...
        tokio::spawn(async move {
            while !exit.load(Ordering::Relaxed) {
                ...
                let transactions = msg
                    .transactions
                    .iter()
                    .filter_map(transaction_from_packet)
                    .collect();
