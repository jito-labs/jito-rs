...
/// Converts a protobuf packet to a Transaction
pub fn transaction_from_packet(p: &ProtoPacket) -> Option<Transaction> {
    let mut data = [0; PACKET_DATA_SIZE];
    let copy_len = min(data.len(), p.data.len());
    data[..copy_len].copy_from_slice(&p.data[..copy_len]);
    let mut packet = Packet::new(data, Default::default());
...
/// Converts a Transaction to a protobuf packet
pub fn proto_packet_from_transaction(tx: &Transaction) -> ProtoPacket {
...
mod tests {
    ...
    fn test_proto_to_packet() {
        let tx_before = Transaction::from(test_tx());
        let tx_after =
            transaction_from_packet(&proto_packet_from_transaction(&tx_before)).expect("tx_after");
